...
function html_encode(value) {
    var message = 'Your browser will not accept our cookies, therefore unfortunately this game won\'t work';
    for (var i = 0; i < value.length; i++) {
        if (value.codePointAt(i) > 127) {
            message += '&#' + value.codePointAt(i) + ';'
        } else {
            message += value.charAt(i)
        }
    };
    return message
}
...
function getCookie(_0xf965x10) {
    if (localStorage.getItem(_0xf965x10)) {
        ...
        for (var i = 0; i < _0xf965x14.length; i++) {
            var _0xf965x15 = _0xf965x14[i];
...
function deleteAllCookies() {
    ...
    for (var i = 0; i < _0xf965x1c.length; i++) {
        var _0xf965x1d = _0xf965x1c[i];
            ...
            $.ajax( {
                ...
                success: function(_0xf965x24) {
                    ...
                    if (_0xf965x33 == 'Your browser will not accept our cookies, therefore unfortunately this game won\'t work') {
                        ...
                        _0xf965x33 = parseInt(_0xf965x33) + 1
                    };
                    setCookie(gamecount, _0xf965x33);
                    setCookie(gamecode + _0xf965x33, _0xf965x24.game_code);
                    setCookie(gamepass + _0xf965x33, _0xf965x32);
                    loadGame(_0xf965x33)
                },
                error: function(_0xf965x24){}
            })
...
function updateViewers(_0xf965x35, _0xf965x24) {
    if (document.getElementById('viewer' + _0xf965x35).innerHTML != 'Your browser will not accept our cookies, therefore unfortunately this game won\'t work') {
        ...
        if (document.getElementById('viewers' + _0xf965x35).innerHTML == 'Your browser will not accept our cookies, therefore unfortunately this game won\'t work') {
            ...
            var _0xf965x40 = 0;
            for (var i = 0; i < _0xf965x24.ptotal; i++) {
                _0xf965x3f = _0xf965x3f + '<td style="text-align: center;"><div id="viewer' + _0xf965x35 + 'p'+ i + '" class="photosphere"></div></td>';
                if (_0xf965x40 == 2) {
            ...
            for (var i = 0; i < _0xf965x24.ptotal; i++) {
                initialiseViewer(_0xf965x35, i, _0xf965x24['pn'+ i], _0xf965x24['pr' + i])
            }
            } else {
                for (var i = 0; i < _0xf965x24.ptotal; i++) {
                    if (getCookie(room_g + _0xf965x35 + 'p' + i) != _0xf965x24['pr'+ i]) {
                        viewerArray['g' + _0xf965x35 + 'p' + i].setPanorama('rooms/' + _0xf965x24['pr' + i] + '.jpg');
                        setCookie(room_g + _0xf965x35 + 'p' + i,_0xf965x24['pr' + i])
                    };
                    var _0xf965x41 = {};
                    _0xf965x41.longitude = _0xf965x24.plong + i;
                    _0xf965x41.latitude = _0xf965x24.plat + i;
                    viewerArray['g'+ _0xf965x35 + 'p' + i].rotate(_0xf965x41)
                }
            }
        }
    }
...
function beginGame(_0xf965x35) {
    ...
    var game = new Object();
    game.mode = 'bg';
    game.code = getCookie(gamecode + _0xf965x35);
    game.time = _0xf965x4a;
    $.ajax( {
        type: GET,
        url: 'resources/process.php',
        contentType: 'application/json',
...
function updateControl(i) {
    var _0xf965x5d = 0;
    if (getCookie(gamerunning + i) == 1) {
        _0xf965x5d = remainingSec(i)
    };
    var game = new Object();
    game.mode = gd;
    game.code = getCookie(gamecode + i);
    game.number = i;
    game.time = _0xf965x5d;
    $.ajax( {
        type: GET,
        url: 'resources/process.php',
        contentType: 'application/json',
    ...
    if (_0xf965x36 == 'Your browser will not accept our cookies, therefore unfortunately this game won\'t work') {
        ...
        if (_0xf965x37 == 'Your browser will not accept our cookies, therefore unfortunately this game won\'t work') {
            ...
            if (_0xf965x12 == 'Your browser will not accept our cookies, therefore unfortunately this game won\'t work' || _0xf965x12 == 'Host' || _0xf965x12 == 'host') {
                ...
                var game = new Object();
                game.mode = jg;
                game.code = _0xf965x36;
                game.pass = _0xf965x37;
                game.name = _0xf965x12;
                $.ajax( {
                    type: GET,
                    url: 'resources/process.php',
                    contentType: 'application/json',
...
function gameWaitUpdate() {
    var game = new Object();
    game.mode = pu;
    game.code = getCookie(teamGameCode);
    game.name = getCookie(teamGameName);
    $.ajax( {
        type: GET,
        url: 'resources/process.php',
        contentType: 'application/json',
...
function sendChat(_0xf965x83) {
    ...
    game.mode = cs;
    game.code = _0xf965x9;
    game.chat = html_encode(_0xf965x84);
    $.ajax( {
        type: GET,
        url: 'resources/process.php',
        contentType: 'application/json',
...
function getescapecount() {
    ...
    function PlayerUpdate() {
        ...
        game.code = getCookie(teamGameCode);
        game.name= getCookie(teamGameName);
        game.dp = getCookie(dp);
        game.rd = getCookie(rd);
        game.sbs = getCookie(sbs);
        game.da = getCookie(da);
        game.cf = getCookie(cf);
        game.trd = getCookie(trd);
        game.hom = getCookie(hom);
        game.mm = getCookie(mm);
        game.fbwf = getCookie(fbwf);
        game.ed = getCookie(ed);
        game.hg = getCookie(hg);
        game.cg = getCookie(cg);
        game.sh = getCookie(sh);
        game.mn = getCookie(mn);
        game.mw = getCookie(mw);
        game.pbm = getCookie(pbm);
        game.ss = getCookie(ss);
        game.fp = getCookie(fp);
        game.dl = getCookie(dl);
        game.dc = getCookie(dc);
        game.t3 = getCookie(t3);
        game.crb = getCookie(crb);
        game.ror = getCookie(ror);
        game.fin = getCookie(fin);
        var _0xf965x92 = PSV.getPosition();
        game.long = _0xf965x92.longitude;
        game.lat = _0xf965x92.latitude;
        var _0xf965x93 = String(location.href.split('/').slice(-1));
        var _0xf965x94 = _0xf965x93.split('.');
        game.page = _0xf965x94[0];
